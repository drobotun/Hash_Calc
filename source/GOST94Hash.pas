...
type
  TGOST94Digest = array [0..31] of byte;

function GOST94Stream(const Stream: TStream) : TGOST94Digest;
function GOST94File(FileName : string): TGOST94Digest;
function GOST94DigestToStr(const Digest: TGOST94Digest): string;
...
var
  ...
  Gost_T: DWord;
begin
   Move( State[0], U[0], SizeOf( U ) );
   Move( Data[0], V[0], SizeOf( V ) );
   i := 0;
   while i < 8 do
   begin
      // X(w, u, v)
...
procedure GOST94Update( const Buf; Size: LongWord );
var
  i, R: Integer;
...
function GOST94GetHash : TGOST94Digest;
var
...
begin
   ...
   GOST94Update( Bytes[0], Stream.Size );
   Result := GOST94GetHash;
end;
